...
$(function() {
	...
	$(window).on('resize',function(){
	 ...
	 $('.list_ico').css({marginLeft:((($('.list_ico li').outerWidth()+30)*5)/2)*-1});
	...
	$(window).on('load',function(){
		...
		$('.page.second .cycle-slideshow,.page.first .cycle-slideshow').on('cycle-before',function(event, optionHash, outgoingSlideEl, incomingSlideEl, forwardFlag) {
		   $(incomingSlideEl).prev().addClass('prev');
		});
		$('.page.second .cycle-slideshow,.page.first .cycle-slideshow').on('cycle-after',function(event, optionHash, outgoingSlideEl, incomingSlideEl, forwardFlag) {
	...
	$('form input,form textarea').focus(function(){
		$(this).after("<div class='foucs_bullet' style='width:2px;height:8px;background:#b1986a;position:absolute;left:"+$(this).position().left+
		"px;top:"+($(this).position().top+$(this).outerHeight()-8)+"px'></div>");	
		$(this).after("<div class='foucs_bullet' style='width:2px;height:8px;background:#b1986a;position:absolute;left:"+($(this).position().left+$(this).outerWidth()-2)+
		"px;top:"+($(this).position().top+$(this).outerHeight()-8)+"px'></div>");
	});
...
function scrollTop(pageNum){
	...
	//'easeInQuint'
	$('#content .page').eq(pageNum).animate({'top': '0%'}, 1000 , function (){
		oldTop(pageNum);
		sectionAni(pageNum);
	});
	$('#content .page').eq(oldNum).animate({'top': '100%'}, 1000, function(){
	});
...
function sectionAni(pageNum){
	...
	var color = ['#fcd182','#99845d','#facb74','#99845d','#99845d','#99845d','#99845d' ];
		
	$('#pop_menu').fadeOut(1000);
	$('#nav-icon').removeClass('open');
	$('#pop_menu li a').removeClass('on');
	if(pageNum-1 >-1){
		$('#pop_menu li').eq(pageNum-1).find('a').addClass('on');
	}
...
function gnbMove(linkNum){
	if(!isMobile){
		...
		body.stop().animate({scrollTop:top_pos}, 'slow', 'linear', function() { 
